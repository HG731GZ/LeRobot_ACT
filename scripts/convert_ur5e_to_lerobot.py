...
def compute_passthrough_video_stats(video_path: Path, length: int, tools: dict) -> dict:
    ...
    return {
        key: value if key == "count" else value.squeeze(axis=0) / 255.0
...
def create_passthrough_dataset(args: argparse.Namespace, episodes: list[EpisodeInfo], fps: int):
    ...
    meta = LeRobotDatasetMetadata.create(
        ...
        features=features,
        use_videos=True,
        # LeRobot's default metadata buffer flushes every 10 episodes and
        # appends to one ParquetWriter. Some per-episode stats can infer
        # slightly different Arrow types between flushes, so write metadata in
        # a single flush for passthrough conversion.
        metadata_buffer_size=max(1, len(episodes)),
    )
